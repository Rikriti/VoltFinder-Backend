...
// Add a favorite charging station
exports.addFavorite = async (req, res) => {
  const { stationId, stationName, location } = req.body;

  try {
    const existingFavorite = await Favorite.findOne({ user: req.user.id, stationId });
    if (existingFavorite) return res.status(400).json({ message: "Station already in favorites" });
...
// Remove a favorite station by its ID
exports.removeFavorite = async (req, res) => {
  try {
    const favorite = await Favorite.findOneAndDelete({ user: req.user.id, _id: req.params.id });
